<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Playlist Information') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <h1 class="font-bold text-3xl pb-2 ml-4">{{ $playlist[0]->playlists->name }}</h1>
                    <div class="divider mb-1 pb-1 border-gray-300 border-solid border-t"></div>

                    @foreach($playlist as $song)
                        <div id="songsParent" class="transition ease-in-out duration-300 hover:bg-gray-200 text-2xl grid grid-rows-1 grid-cols-12 p-6 bg-white border-b border-gray-200">
                            <div id="test" class="row-start-1 col-start-1 col-end-4 border-r-2 border-gray-200">
                                <h2 class="font-bold">{{ $song->songs->name }}</h2>
                                <h5 class="italic">{{ $song->songs->artist }}</h5>
                            </div>

                            <div class="grid relative flex items-center justify-center row-start-1 col-start-4 col-end-6">
                                <small class="text-xs absolute top-1 left-2">Genre</small>
                                <p class="italic underline">
                                    {{ $song->songs->genre->name }}
                                </p>
                            </div>

                            <div class="flex items-center justify-center row-start-1 col-start-7 col-end-9">
                                <p class="icons">
                                    <i class="far fa-play-circle"></i> {{ date("i:s", strtotime($song->songs->duration)) }}
                                </p>
                            </div>

                            <div class="flex items-center justify-center row-start-1 col-start-11 col-end-13">
                                <p>
                                    <a href="{{ route('playlists.deleteS', $song->id) }}"><i class="far fa-trash-alt cWidth"></i></a>
                                    {{-- =>> $song->id just a placeholder, id of the SongsPlaylist DB should come here. Somehow..--}}
                                </p>
                            </div>
                        </div>
                    @endforeach
                <div class="py-3 px-3 bg-gray-100 flex justify-center text-lg rounded">
                    <p>
                        Total playlist time: {{ $duration }}
                    </p>
                </div>
                </div>
            </div>
        </div>
    </div>
</x-app-layout>
